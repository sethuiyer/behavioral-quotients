...
function visibleText(node, { math = 'placeholder', code = false } = {}) {
  ...
  (function rec(n) {
    for (const c of kids(n)) {
      ...
      if (hasCls(c, 'd-byline') || hasCls(c, 'd-audio')) continue;
      if (t === 'pre' || t === 'code') {
...
// --- T12: local subresources exist on disk ---------------------------------
// Chrome that is generated by the build (the audio bar, favicon, OG card,
// vendored MathJax) is not in the markdown, so the correspondence tests cannot
// see it. This asserts every local file the page references is actually there
// and non-empty — a missing lecture MP3 is a 404 at runtime, not a typo.

test('T12', 'local assets exist', () => {
  const SUBRESOURCE_REL = new Set([
    'stylesheet', 'icon', 'shortcut icon', 'apple-touch-icon', 'mask-icon', 'manifest',
  ]);
  const missing = [];
  let checked = 0;
  for (const el of findAll(doc, (n) =>
    ['script', 'link', 'img', 'source', 'audio', 'video'].includes(n.tagName)
  )) {
    if (el.tagName === 'link') {
      const rel = (attr(el, 'rel') || '').toLowerCase();
      if (!SUBRESOURCE_REL.has(rel)) continue;
    ...
    checked++;
    const p = path.join(SITE_DIR, url.split('?')[0].split('#')[0]);
    if (!fs.existsSync(p) || fs.statSync(p).size === 0) missing.push(url);
  }
  const observed = { checked, missing: missing.length };
  if (missing.length) {
    return {
      ok: false,
      detail: `${missing.length} referenced local asset(s) missing or empty: ${missing.join(', ')}`,
      observed,
    };
  }
  return { ok: true, detail: `${checked} local subresource(s) present on disk (incl. lecture audio)`, observed };
});
